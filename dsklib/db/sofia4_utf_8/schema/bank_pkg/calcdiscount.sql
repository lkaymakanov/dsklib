DROP FUNCTION  IF EXISTS  bank_pkg.calcdiscount(numeric, numeric, date, character varying, numeric); 
CREATE OR REPLACE FUNCTION bank_pkg.calcdiscount(iptaxsubject_id numeric, iptaxobject_id numeric, ippaydate date, OUT opstat character varying, ippartno character varying, ipkinddebtreg_id numeric)
...
AS $function$
  ...
  FROM
  	...
  	LEFT JOIN (
  	     	SELECT
  	     		...
  	     		ds.taxperiod_id,
  	     		ds.kinddebtreg_id,
  	     		MAX (COALESCE (pd.paydate, to_date ('1970-01-01'))) paydate,
  	     	...
  	     	SELECT
  	     		...
  	     		ds.taxperiod_id,
  	     		ds.kinddebtreg_id,
  	     		MAX (COALESCE (o.oper_date, to_date ('1970-01-01'))) paydate,
  ...
  HAVING
  	...
  	AND coalesce(max(pd.paydate),to_date('1970-01-01')) <= d.termdisc -- дата последното плащане,за което е направено отстъпка <= срок на отстъпка????
  ...
  WHERE	ds.taxsubject_id =  iptaxsubject_id
  	...
  	AND (
  	    	(
  	    		to_char (COALESCE (td.earn_date, to_date ('1970-01-01')), 'yyyy') 
  		...
  		( 
  			...
  			AND (COALESCE (td.earn_date, to_date ('1970-01-01')) > COALESCE (tr.paidpodate,to_date ('1970-01-01'))) 
  			)
  	...
  	AND ( 
  		...
  		OR 
  		( td.endtaxdate >= to_date(to_char(trunc (ippaydate), 'yyyy')||'-12-31') ) 
  		--закрити след
  	)
  	AND ds.kinddoc = '1'
  GROUP BY
  ...
  BEGIN
  	...
  	SET
  		discsum = 0
  	WHERE
  		debtinstalment_id IN (SELECT
  		                      	b.debtinstalment_id
  		                      FROM
  		                      	debtsubject ds
  		                      	JOIN debtinstalment di
  		                      		ON   ds.debtsubject_id = di.debtsubject_id
  		                      	LEFT JOIN baldebtinst b
  		                      		ON   b.debtinstalment_id = di.debtinstalment_id
  		                      WHERE
  		                      	ds.taxsubject_id = iptaxsubject_id);
  		FOR rdebt IN cr
  		LOOP
  				UPDATE
  					baldebtinst bdi
  				SET
  					discsum = rdebt.discsum
  				WHERE
  					bdi.debtinstalment_id = rdebt.debtinstalment_id;
  		END LOOP;
